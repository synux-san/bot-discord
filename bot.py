...
from module import channel_creation
...
async def create(ctx, *to_invit: discord.Member):
    ...
    def check(reaction, user):
        ...
        return ((str(reaction.emoji) == module.emoji_yes) or (str(reaction.emoji) == module.emoji_no) or str(
            reaction.emoji) == module.emoji_close)
        #	--------------	#
        ...
        try:
            ...
            if (reaction.message.id == message.id and reactor in to_invit):
                if reactor.guild_permissions.administrator:
                    await ctx.send(
                        f"Désolé {reactor.name}, les membres avec la permission administrateur activée ne peuvent pas jouer car leur rôle permet de voir les salons privés. {module.emoji_sad}")
    ...
    if len(players) < min_player:
        return await ctx.send(
            f"Il faut au minimum {min_player} joueurs. Reviens une prochaine fois avec plus de joueur")
    ...
    if len(players) > 6:
        ...
        embed.add_field(name="\uFEFF", value=role_message(player=player_list[0], role="entrainé", server=ctx.guild.name,
                                                          invite=villager_channel.create_invite()))
    ...
    for villager in villagers:
        ...
        embed = discord.Embed(title="Information", color=0x35874f)
        embed.add_field(name="\uFEFF", value=role_message(player=villager, role="villageois", server=ctx.guild.name,
                                                          invite=villager_channel.create_invite()))
    ...
    for impostor in impostors:
        ...
        embed = discord.Embed(title="Information", color=0x35874f)
        embed.add_field(name="\uFEFF", value=role_message(player=impostor, role="imposteur", server=ctx.guild.name,
                                                          invite=impostor_channel.create_invite()))
    ...
    while (villagers != [] and impostors != []):
        ...
        if (night_or_day % 2) == 0:
            ...
            await impostor_channel.send(
                f"Il fait nuit. Il est l'heure de proceder au vote pour sacrifier un villageois {module.emoji_devil}")
            ...
            if vote != {}:
                ...
                if (((night_or_day / 2) % 2) == 0 and players[0] == killed):  # au nuit paire
                    ...
                    revenge = await player[0].send(
                        "{players[0].name}, {first_match_killer.name} ap voter pour votre mort. Que feras-tu?\n{module.emoji_yes} : pour le laiser en vie\n{module.emoji_no} : pour l'éliminer.\nVous avez 10 secondes")
                    ...
                    try:
                        ...
                        if reaction.emoji == module.emoji_yes:
                            ...
                            await module.kill_perm(channel=impostor_channel, member=first_match_killer)
                            await module.kill_perm(channel=villager_channel, member=first_match_killer)
                            await villager_channel.send(f"RIP, {first_match_killer.name} ap été éliminé(e)")
                villagers.remove(killed)
                await module.kill_perm(channel=villager_channel, member=killed)
                await villager_channel.send(f"RIP, {killed.name} ap été éliminé(e).")
            else:
                await villager_channel.send("Les imposteurs w'ont pas attaqué le village cette nuit.")

            ### ===le jour=== ###
        else:
            await villager_channel.send(
                f"...Et le jour parvint {module.emoji_day}. Les imposteurs ne sont pas tous exterminés. Qui pensent-tu être le plus susceptible d'être imposteur? Place au vote.")
            ...
            if vote != {}:
                killed = max(list(vote.values()), key=list(vote.values()).count)
                villagers.remove(killed)
                await villager_channel.send(f"{killed.name} ap été eliminé(e)")
                ...
                if killed in villager:
                    ...
                    await module.kill_perm(channel=impostor_channel, member=killed)
            else:
                await villager_channel.send("Personne w'as été éliminé(e)")
    ...
    if role != "":
        ...
        if role.lower() == "villageois" or role.lower() == "vil":
            ...
            embed.add_field(name="Description : ",
                            value=" Il vote le jour pour eliminer un autres villageois suspect d'être un imposteur\nIl gagne la partie si tous les imposteurs sont éliminés")
        elif role.lower() == "imposteur" or role.lower() == "imp":
            embed.add_field(name="Imposteur", value="Alias : imp")
            embed.add_field(name="Description : ",
                            value="Il est avant tout un villageois. Il peut voter la nuit pour éliminer un villageois "
                                  "et le jour pour en éliminer un autre. son rôle est de defendre ses semblables pour "
                                  "qu'il soit en vie jusqu'à la fin du jeu.\nIl gagne la partie si tous les "
                                  "villageois sont éliminés.")
        elif role.lower() == "entrainé" or role.lower() == "entraine" or role.lower() == "ent":
            embed.add_field(name="L'entrainé", value="Alias : ent")
            embed.add_field(name="Description : ",
                            value="Description : Au nuit paire, si il obtient plus de vote pour mourir, il peut tuer un des imposteurs qui a voté pour sa mort.")
        else:
            embed.add_field(name="\uFEFF", value=f"Le rôle {role.capitalize()} n'existe pas")
...
async def invite(ctx):
    await ctx.send("https://discord.com/api/oauth2/authorize?client_id=797677986144714782&permissions=2080898161"
                   "&scope=bot")
...
async def bug(ctx, *, report):
    ...
    await ctx.send(f"Merci {ctx.author.name}, le bug ap été bien envoyé.")
...
async def help(ctx, arg=""):
    ...
    embed.add_field(name="Rappel", value="Les crochets tels que [] ou <> ne sont pas à utiliser lors de l'execution "
                                         "des commandes.")
    if arg == "":
        pass

    elif arg == "create":
        embed.add_field(name="Commande Create", value="Description: permet de créer un nouveau jeu. Les membres avec "
                                                      "la permission administratreur activé ne peuvent pas jouer.")
        ...
        embed.add_field(name="\uFEFF", value="La commande {arg} w'est pas trouvée.")
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.NoPrivateMessage):
        await ctx.author.send(f"{ctx.author.name}, cette commande w'est pas disponible en messages privés")

    elif isinstance(error, commands.errors.CheckFailure):
        await ctx.send(f"{ctx.author.name}, tu w'as pas le droit de faire ça")

...
async def vote_message(channel, players):
    ...
    for page in range(ceil(len(players) / 10)):
        ...
        for emoji_n, player in dict(
                zip(emoji_vote()[:len(players[start_slice:end_slice:])], players[start_slice:end_slice:])).items():
    ...
    while looking_for_react:
        ...
        try:
            ...
            if total_voted == len(players):
                await channel.send("Tous le monde ap déjà voté")
                looking_for_react = False
    return vote
...
async def channel_creation(ctx, channel_name, members):
    ...
    await channel.set_permissions(ctx.guild.me, send_messages=True, read_messages=True, add_reactions=True,
